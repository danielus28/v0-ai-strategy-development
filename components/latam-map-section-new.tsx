...
export function LatamMapSection() {
    ...
    // Load SVG and setup event listeners
    useEffect(() => {
        const loadSVG = async () => {
            ...
            try {
                ...
                if (svg) {
                    ...
                    circles.forEach((circle) => {
                        circle.addEventListener('mouseenter', handleCircleHover as EventListener)
                        circle.addEventListener('mouseleave', handleCircleLeave)
                        circle.addEventListener('click', handleCircleClick as EventListener)
                    })
                }
        ...
        return () => {
            if (svgContainerRef.current) {
                ...
                if (svg) {
                    const circles = svg.querySelectorAll('[data-country]')
                    circles.forEach((circle) => {
                        circle.removeEventListener('mouseenter', handleCircleHover as EventListener)
                        circle.removeEventListener('mouseleave', handleCircleLeave)
                        circle.removeEventListener('click', handleCircleClick as EventListener)
                    })
                }
            }
        }
    ...
    // Update colors when layer changes
    useEffect(() => {
        ...
        circles.forEach((circle) => {
            ...
            if (country) {
                const color = getColorForLayer(country, activeLayer)
                    ...
                    ; (circle as SVGCircleElement).style.transition = 'fill 0.3s ease'
            }
        })
    }, [activeLayer, svgLoaded])

    // Update hover/select styles
    useEffect(() => {
        if (!svgLoaded || !svgContainerRef.current) return
